---
# =========================================================
# ■ 基本識別情報（必須）
# =========================================================

layout: "work.njk"
work_code: "SPSD-68X"
title: "ヒロイン変身失敗 ウェネト"
permalink: "/{{ work_code }}/"

# 公開設定
draft: true


# =========================================================
# ■ ジャンル（1つ以上残す・不要なものを削除）
# slugは genresData.json と一致させる
# =========================================================

genres:
  - battle-suit


# =========================================================
# ■ タグ（自由入力）
# tagsData.json を参照
# 例: tentacle, crucifixion, mind-break など
# =========================================================

tags:
  - tag-slug-1
  - tag-slug-2


# =========================================================
# ■ ビジュアル設定
# =========================================================

keyvisual: "images/SPSD-68_Wenet_Heroine.jpg"

# 単色例: "rgb(250, 20, 20)"
# 二色例: "linear-gradient(135deg, #f0bc68 50%, #c4d7d1 50%)"
keycolor: "rgb(250, 20, 20)"


# =========================================================
# ■ 基本情報（必須）
# =========================================================

actress:
  - 宇流木さらら

# slug形式（例: Yaya-Kohaku）
actress_slugs:
  - Sarara-Uruki

director: 壬生乃
director_slug: Mibuno

# 数字のみ（分）
duration: "115"

# 形式: YYYY-MM-DD
release_date: "2025-06-04"


# =========================================================
# ■ Schema.org用（SEO補助）
# =========================================================

# 例: "4.5"
schema_rating: "4.0"

# 形式: YYYY-MM-DD
date_published: "2026-02-18"

schema_review_body: "作品全体の要約を200文字程度で入力"


# =========================================================
# ■ STORYセクション
# HTMLで記述（pタグ必須）
# =========================================================

story_lead: |
  <p>科学研究所で極秘に開発された人間をコントロールするツール「ドレイ・ドライバー」。</p>
  <p>コントロールされた者は「ドレイラー」と呼ばれ恐れられていた。</p>
  <p>政府は極秘裏にドレイラーを排除するため、組織A.R.M.E.D（アームド）を創立する。</p>
  <p>アーム度の捜査官でドレイラーと日々戦う、ウェネトこと兎川未希。</p>
  <p>彼女は敵の罠によって変身失敗してしまい、戦闘中にあられもない姿となってしまう。</p>
  <p>絶対絶命のピンチにドレイ・ドライバーの開発者、独朗よりすべての真実が語られる…。</p>
  <p>愕然とする未希を襲う絶望へのカウントダウン！</p>
  <p>果たして未希・ウェネトに勝機はあるのか！？</p>
  <p>「コードネーム・ウェネト！治安維持特別法違反を確認、対象を殲滅する！」</p>


# =========================================================
# ■ 動画ソース
# =========================================================

# ローカル研究用
# default_video_src: "/videos/SPSE-69.mp4"

# 本番用（R2等）
default_video_src: "https://example.com/SPSE-69.mp4"


# =========================================================
# ■ SCENESパネル
# start/end は "00:01:23" 形式推奨
# Type: S/T align: L/C/R tail: L/C/R/N 
# =========================================================

scenes:
  -
    start: "00:00:10"
    end: "00:00:20"
    top:
      - { type: "S", align: "L", tail: "R", text: "解説テキスト" }
    bottom: []

  -
    start: "00:05:30"
    end: "00:05:40"
    top:
      - { type: "T", align: "C", tail: "C", text: "ハイライト解説" }
    bottom: []


# =========================================================
# ■ REVIEW構造
# reviewers.json を参照
# id: critic/judge/madame/dr/analyzer/star/noir/bot
# =========================================================

review:

  reviewers:
    judge:
      position: "left"
      description: "構造・論理を分析"

    madame:
      position: "right"
      description: "官能・屈服を分析"

  sections:

    - title: "序論"
      dialogue:
        - speaker: "judge"
          text: "分析テキスト"
        - speaker: "madame"
          text: "官能テキスト"

    - title: "結論"
      dialogue:
        - speaker: "judge"
          text: "総括"
        - speaker: "madame"
          text: "官能的結語"

---
